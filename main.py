...
class App(FramelessWindow):
    ...
    def build_ui(self):
        ...
        gear.setIcon(QIcon(os.path.join(getattr(sys, "_MEIPASS", os.path.dirname(__file__)), "assets", "gear.svg")))
        gear.setIconSize(QPixmap(20, 20).size())
        gear.setToolTip("Export folder")
        gear.setFixedSize(38, 38)
        gear.setStyleSheet(f"QPushButton#gearButton {{ background:transparent; border:0; padding:8px; }} QPushButton#gearButton:hover {{ background:{PANEL}; border-radius:7px; }}")
        gear.clicked.connect(self.open_settings)
        minimize = QPushButton("—")
        minimize.setObjectName("minimizeButton")
        minimize.setFixedSize(34, 34)
        minimize.setStyleSheet(f"QPushButton#minimizeButton {{ background:transparent; border:0; color:{DIM}; font-size:18px; padding:0; }} QPushButton#minimizeButton:hover {{ background:{PANEL}; border-radius:7px; color:{TEXT}; }}")
        minimize.clicked.connect(self.showMinimized)
        close = QPushButton("×")
        close.setObjectName("closeButton")
        close.setFixedSize(34, 34)
        close.setStyleSheet(f"QPushButton#closeButton {{ background:transparent; border:0; color:{DIM}; font-size:21px; padding:0; }} QPushButton#closeButton:hover {{ background:#5a2525; border-radius:7px; color:white; }}")
